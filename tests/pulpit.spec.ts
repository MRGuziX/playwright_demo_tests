...
test.describe(' Pulpit tests ',() => {
    ...
    test('mobile money transfer', async ({ page }) => {
        ...
        await expect(page.locator('#widget_1_topup_agreement').isChecked()).toBeTruthy();

        await page.getByRole('button', { name: 'doładuj telefon' }).click();
        await page.getByTestId('close-button').click();

        await expect(page.getByTestId('message-text')).toHaveText('Doładowanie wykonane! 11,00PLN na numer 500 xxx xxx');
    });
    ...
    test('mobile top-up warning test', async ({ page }) => {
        ...
        await expect(page.locator('#widget_1_topup_agreement').isChecked()).toBeTruthy();
        await expect(page.getByTestId('error-widget-1-topup-receiver')).toBeHidden();
        await expect(page.getByTestId('error-widget-1-topup-amount')).toBeHidden();
        await expect(page.getByTestId('error-widget-1-topup-agreement')).toBeHidden();
    });
    ...
    test('mobile top-up amount warning test', async ({ page }) => {
        ...
        await expect.soft(page.getByTestId('error-widget-1-topup-amount')).toHaveText("kwota musi być większa lub równa 5");

        await page.locator('#widget_1_topup_amount').fill('5');
        await page.locator('#widget_1_topup_amount').blur();

        await expect.soft(page.getByTestId('error-widget-1-topup-amount')).toBeHidden();
        ...
        await expect.soft(page.getByTestId('error-widget-1-topup-amount')).toHaveText("kwota musi być mniejsza lub równa 150");

        await page.locator('#widget_1_topup_amount').fill('50,01');
        await page.locator('#widget_1_topup_amount').blur();

        await expect.soft(page.getByTestId('error-widget-1-topup-amount')).toHaveText("kwota musi być bez groszy");

        await page.locator('#widget_1_topup_amount').fill('50,,01');
        await page.locator('#widget_1_topup_amount').blur();

        await expect.soft(page.getByTestId('error-widget-1-topup-amount')).toHaveText("podana kwota jest niepoprawna");
    });
});